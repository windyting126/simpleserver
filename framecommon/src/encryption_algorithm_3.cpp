// encryption_algorithm_3.cpp: implementation of the encryption_algorithm_3 class.
//
//////////////////////////////////////////////////////////////////////
...
bool encryption_algorithm_3::create_key( const unsigned char *data, unsigned len, void *key, unsigned &key_len )
{
	// 创建首部
	if (len == 0)
	{
		...
		// 加密方法
		unsigned head = ((8192*3 + rand() % 8192) << 16) + rand() % 65536; // [0x60000000, 0x7FFFFFFF]
		head = htonl( head);
		memcpy(_key, &head, 4);
		_key_len += 4;
...
bool encryption_algorithm_3::encrypt( unsigned char *in_buffer, unsigned len )
{
	unsigned next;
	for (unsigned i=0; i<len; i++)
	{
		...
		in_buffer[i] = _key[_key_pos] - in_buffer[i];
		_key_pos = next;
	}
	return true;
}

bool encryption_algorithm_3::decrypt( unsigned char *in_buffer, unsigned len )
{
	unsigned next;
	for (unsigned i=0; i<len; i++)
	{

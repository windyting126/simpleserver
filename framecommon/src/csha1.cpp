...
void ctx_sha1::handle(unsigned int *state, const unsigned char data[64])
{
...
void ctx_sha1::update(const unsigned char *pdata, unsigned int count)
{
    if(0 < count)
	{
	    unsigned int left = _count[0] & 0x3F;
...
void ctx_sha1::finish(unsigned char cid[20])
{
    ...
    unsigned char ml[8];	
    unsigned int hi = ( _count[0] >> 29 ) | ( _count[1] <<  3 );
    unsigned int lo = ( _count[0] <<  3 );

    PUT_UINT32(hi, ml, 0);
    PUT_UINT32(lo, ml, 4);

    unsigned int last = _count[0] & 0x3F;

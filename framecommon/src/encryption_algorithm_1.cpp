// encryption_algorithm_1.cpp: implementation of the encryption_algorithm_1 class.
//
//////////////////////////////////////////////////////////////////////
...
bool encryption_algorithm_1::create_key( const unsigned char *data, unsigned len, void *key, unsigned &key_len )
{
	// 创建首部
	if (len == 0)
	{
		...
		// 加密方法
		unsigned head = ((8192*1 + rand() % 8192) << 16) + rand() % 65536; // [0x20000000, 0x3FFFFFFF]
		head = htonl( head);
		memcpy(_key, &head, 4);
		_key_len += 4;
...
bool encryption_algorithm_1::encrypt( unsigned char *in_buffer, unsigned len )
{
	unsigned next;
...
bool encryption_algorithm_1::decrypt( unsigned char *in_buffer, unsigned len )
{
	unsigned next;
	for (unsigned i=0; i<len; i++)
	{
